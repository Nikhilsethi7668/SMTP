...
const isWithinSchedule = (schedule, campaignTimezone) => {
  ...
  const rawTz = schedule.timezone || campaignTimezone || process.env.TZ || "UTC";
  const tz = normalizeTimezone(rawTz);
  const now = moment().tz(tz);
  
  console.log(`   🕐 Current time in ${tz}: ${now.format("YYYY-MM-DD HH:mm:ss dddd")}`);

  if (schedule.start_date) {
    const start = moment(schedule.start_date).tz(tz).startOf("day");
    const isAfterStart = now.isSame(start, "day") || now.isAfter(start);
    console.log(`   📅 Start date check: ${now.format("YYYY-MM-DD")} >= ${start.format("YYYY-MM-DD")}? ${isAfterStart}`);
    if (!isAfterStart) {
  ...
  if (schedule.end_date) {
    ...
    console.log(`   📅 End date check: ${now.format("YYYY-MM-DD")} <= ${end.format("YYYY-MM-DD")}? ${isBeforeEnd}`);
    if (!isBeforeEnd) {
  ...
  if (Array.isArray(schedule.days) && schedule.days.length > 0) {
    ...
    console.log(`   📆 Day check: Today is "${dayName}", allowed days: [${schedule.days.join(", ")}]`);
  ...
  if (schedule.from_time && schedule.to_time) {
    ...
    console.log(`   ⏰ Time window check: Current=${currentTime}, Window=${from}-${to}`);
    ...
    if (from === to) {
      ...
      console.log(`   ${isWithin ? '✅' : '❌'} Time check: ${currentTime} is ${isWithin ? 'within' : 'outside'} window ${from}-${to}`);
      if (!isWithin) return false;
    } else {
      // Overnight window (e.g., 22:00 - 06:00)
      const isWithin = currentTime >= from || currentTime <= to;
      console.log(`   ${isWithin ? '✅' : '❌'} Overnight time check: ${currentTime} is ${isWithin ? 'within' : 'outside'} window ${from}-${to}`);
      if (!isWithin) return false;
    }
  }
...
const scheduleStepForCampaign = async (campaign, schedule, step, variants) => {
  try {
    ...
    console.log(`   📍 Processing step ${step.order} (index ${stepIndex}) - "${step.name || 'Unnamed'}"`);
    ...
    const eligibleLeads = leads.filter(lead => {
      return lead.status === "Not yet contacted" && (lead.current_step || 0) === stepIndex;
    });
    
    console.log(`   ✅ ${eligibleLeads.length} lead(s) eligible (status: "Not yet contacted", current_step: ${stepIndex})`);
    if (eligibleLeads.length < leads.length) {
      console.log(`   ⚠️ ${leads.length - eligibleLeads.length} lead(s) filtered out (wrong status or step)`);
    }

    const toSchedule = Math.min(remaining, eligibleLeads.length);
    console.log(`   🎯 Scheduling ${toSchedule} email(s) (${remaining} remaining, ${eligibleLeads.length} eligible leads)`);
    ...
    for (let i = 0; i < toSchedule; i++) {
      ...
      if (lead.status !== "Not yet contacted") {
        console.log(`   ⚠️ Skipping lead ${lead._id} (${lead.email}) - status is "${lead.status}", not "Not yet contacted"`);
        skippedCount++;
        continue;
      }
      ...
      const variantInfo = variant ? ` (variant ${variants.findIndex(v => v._id.toString() === variant._id?.toString()) + 1})` : "";
      console.log(`   ✓ Job ${job.id} scheduled for ${lead.email}${variantInfo} (delay: ${delayMinutes}m ${delaySecRemainder}s, from: ${fromEmail || 'N/A'})`);
    ...
    if (skippedCount > 0) {
      console.log(`   ⚠️ Skipped ${skippedCount} lead(s) due to invalid status`);
    }

    const newStats = await getTodayCampaignCounts(campaign._id);
    console.log(`   ✅ Step ${step.order} complete: ${scheduledJobs.length} email(s) scheduled`);
    return { scheduled: scheduledJobs.length, jobIds: scheduledJobs, todayStats: newStats, skipped: skippedCount };
  } catch (err) {
...
const processCampaign = async (campaign) => {
  try {
    ...
    if (campaign.status !== "running") {
      console.log(`   ⚠️ Campaign status is "${campaign.status}", not "running" — skipping`);
    ...
    const rawTimezone = campaign.timezone || schedule?.timezone || process.env.TZ || "UTC";
    const timezone = normalizeTimezone(rawTimezone);
    console.log(`   🌍 Timezone: ${rawTimezone}${rawTimezone !== timezone ? ` (normalized to ${timezone})` : ''}`);

    if (schedule) {
      console.log(`   📅 Schedule: ${schedule.name || 'Unnamed'}`);
      if (schedule.start_date) console.log(`      Start: ${moment(schedule.start_date).format('YYYY-MM-DD')}`);
      if (schedule.end_date) console.log(`      End: ${moment(schedule.end_date).format('YYYY-MM-DD')}`);
      if (schedule.days?.length > 0) console.log(`      Days: ${schedule.days.join(', ')}`);
      if (schedule.from_time && schedule.to_time) console.log(`      Time: ${schedule.from_time} - ${schedule.to_time}`);
    } else {
    ...
    for (const step of steps) {
      ...
      console.log(`   📦 Step ${step.order} has ${variants?.length || 0} variant(s)`);
    ...
    console.log(`   ✅ Campaign "${campaign.name}" complete: ${totalScheduled} email(s) scheduled across ${results.length} step(s)`);
    return { processed: 1, scheduled: totalScheduled, results };
  } catch (err) {
    console.error(`   ❌ Error processing campaign ${campaign.name}:`, err.message);
...
export const processRunningCampaigns = async () => {
  try {
    ...
    if (!campaigns || campaigns.length === 0) {
      console.log("ℹ️ No running campaigns found. Make sure you have campaigns with status='running' in the database.");
    ...
    return { success: true, count: campaigns.length, totalScheduled, results: out };
  } catch (err) {
...
export const startCampaignScheduler = (cronExpr = process.env.CAMPAIGN_CRON || "*/5 * * * *") => {
  ...
  processRunningCampaigns().then((result) => {
    ...
    if (result.results && result.results.length > 0) {
      result.results.forEach((r) => {
        if (r.scheduled > 0) {
          console.log(`  ✅ Scheduled ${r.scheduled} email(s) for campaign "${r.campaignName || r.campaignId}"`);
        } else {
          console.log(`  ⚠️ No emails scheduled for campaign "${r.campaignName || r.campaignId}": ${r.message || 'Unknown reason'}`);
        }
      });
    }
  }).catch((error) => {
